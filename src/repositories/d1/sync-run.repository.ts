...
export class D1SyncRunRepository implements SyncRunRepository {
    ...
    async finalize(userId: string, runId: string, status: SyncRunStatus, successCount: number, errorCount: number): Promise<void> {
        await this.db.prepare(`
            UPDATE sync_runs 
            SET status = ?, success_count = ?, error_count = ?, executed_at = unixepoch()
            WHERE user_id = ? AND id = ?
        `).bind(status, successCount, errorCount, userId, runId).run();
    }
