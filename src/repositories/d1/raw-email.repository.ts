...
export class D1RawEmailRepository implements RawEmailRepository {
    ...
    async findById(userId: string, id: string): Promise<RawEmailRow | null> {
        const result = await this.db.prepare('SELECT * FROM raw_emails WHERE user_id = ? AND id = ?').bind(userId, id).first<RawEmailRow>();
        return result;
    }
    ...
    async updateParseStatus(userId: string, id: string, status: ParseStatus): Promise<void> {
        await this.db.prepare("UPDATE raw_emails SET parse_status = ? WHERE user_id = ? AND id = ?")
            .bind(status, userId, id).run();
    }

    async findPending(userId: string): Promise<RawEmailRow[]> {
        const { results } = await this.db.prepare("SELECT * FROM raw_emails WHERE user_id = ? AND parse_status IN (?, ?) ORDER BY fetched_at ASC")
