...
export class EmailParser {
    ...
    private static readonly DATE_REGEX = /【利用日時】(\d{4})年(\d{1,2})月(\d{1,2})日\(.\)(\d{1,2}:\d{2})～(\d{1,2}:\d{2})/;
    ...
    static parse(body: string, subject?: string): ParsedBooking | null {
        ...
        // 4. その他の情報の抽出
        const registrationNumber = body.match(this.REG_NO_REGEX)?.[1].trim();
        const purpose = body.match(this.PURPOSE_REGEX)?.[1].trim();
     ...
     */
    private static formatDateRange(match: RegExpMatchArray): { eventDate: string; eventEndDate: string } {
